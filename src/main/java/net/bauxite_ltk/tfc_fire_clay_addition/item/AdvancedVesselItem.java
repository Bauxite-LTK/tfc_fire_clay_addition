...
import net.bauxite_ltk.tfc_fire_clay_addition.container.TFCFCAContainerProviders;
...
public class AdvancedVesselItem extends Item {
    ...
    public AdvancedVesselItem(Properties properties) {
        super(properties
                .component(TFCFCAComponents.ADVANCED_VESSEL, AdvancedVesselComponent.EMPTY)
        ...
        if (!player.isShiftKeyDown() && !level.isClientSide() && player instanceof ServerPlayer serverPlayer)
        {
            ...
            if (advancedVessel != null)
            {
                if (advancedVessel.isInventory())
                {
                    ...
                    {
                        TFCFCAContainerProviders.ADVANCED_VESSEL.openScreen(serverPlayer, hand);
                    }
                }
